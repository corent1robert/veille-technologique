...
console.log('🔧 Personnalisation des prompts pour chaque chemin avec les bons IDs (Version 3)...');
...
try {
          ...
          if (route.flow && Array.isArray(route.flow)) {
            ...
            route.flow.forEach((module, moduleIndex) => {
              if (module && module.id !== undefined) {
                // Identifier les modules GPT par leur mapper.model
                if (module.mapper && module.mapper.model && 
                    (module.mapper.model === 'o4-mini' || 
                     module.mapper.model === 'gpt-4' ||
                     module.mapper.model === 'gpt-3.5-turbo')) {
                  gptModules.push({
                    index: moduleIndex,
                    id: module.id,
                    name: module.name || 'GPT Module',
                    path: `${routePath}.flow.${moduleIndex}`,
                    model: module.mapper.model
                  });
                }
              }
            });
          ...
          if (path.includes(info.routePath)) {
            ...
            if (obj.mapper && obj.mapper.data) {
              ...
              if (gptIds.length >= 4) {
                ...
                obj.mapper.data = personalizedPrompt;
  ...
  const personalizedBlueprintPath = path.join(__dirname, 'Veille Technologique - Pipeline Complet - PROMPTS-PERSONNALISES-V3.blueprint.json');
