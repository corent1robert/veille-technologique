...
// Chemin vers le blueprint avec prompts personnalisés
const blueprintPath = path.join(__dirname, 'Veille Technologique - Pipeline Complet - PROMPTS-PERSONNALISES-V2.blueprint.json');

console.log('🔧 Correction de TOUTES les références cassées dans le blueprint...');
...
try {
          ...
          if (route.flow && Array.isArray(route.flow)) {
            ...
            route.flow.forEach((module, moduleIndex) => {
              if (module && module.id !== undefined) {
                ...
                // 5. Chercher dans filters (très important !)
                if (module.filters) {
  ...
  const fixedBlueprintPath = path.join(__dirname, 'Veille Technologique - Pipeline Complet - TOUTES-REFERENCES-CORRIGEES.blueprint.json');
