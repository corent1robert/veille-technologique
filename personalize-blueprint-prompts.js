...
console.log('🔧 Personnalisation des prompts pour chaque chemin avec les bons IDs...');
...
try {
          ...
          if (route.flow && Array.isArray(route.flow)) {
            ...
            route.flow.forEach((module, moduleIndex) => {
              if (module && module.id !== undefined) {
                // Identifier les modules GPT par leur nom ou type
                if (module.name && module.name.includes('OpenAI') || 
                    module.name && module.name.includes('GPT') ||
                    module.type && module.type.includes('openai')) {
                  gptModules.push({
                    index: moduleIndex,
                    id: module.id,
  ...
  const personalizedBlueprintPath = path.join(__dirname, 'Veille Technologique - Pipeline Complet - PROMPTS-PERSONNALISES.blueprint.json');
